...
New-UDTypography -Text "Search for users within AD. By default, standard PowerShell filter syntax is used (e.g. Name -like 'j*')."
  New-UDElement -Tag 'p'
  New-UDForm -Content {
    ...
    if ($EventData.LdapFilter) {
      $Session:Objects = Get-ADUser -LdapFilter $EventDat.filter 
    }
    else {
      $Session:Objects = Get-ADUser -Filter $EventData.filter 
    }
